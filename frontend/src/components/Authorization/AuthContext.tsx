...
export const AuthProvider: FC<Interfaces.IAuthProvider<ReactNode>>  = ({ children }) => {
    ...
    const [authTokens, setAuthTokens] = useState<Interfaces.IAuthTokens | null>(() => localStorage.getItem('authTokens') ? authTokensParsed: null)
    const [user, setUser] = useState<string | null>(() => localStorage.getItem('authTokens') ? decodeToken(authTokensParsed.access) : null)
